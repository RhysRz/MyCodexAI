...
const JOB_KINDS = new Set(["health", "agent", "index"]);
...
async function list(context: RequestContext): Promise<Response> {
  ...
  return json({ devices: (rows.results || []).map((row) => ({ ...row, capabilities: JSON.parse(row.capabilities_json || "[]") })) });
}
...
async function register(context: RequestContext): Promise<Response> {
  ...
  await context.env.DB.prepare(
    `INSERT INTO hybrid_devices (id, user_id, name, token_hash, capabilities_json, status, created_at)
     VALUES (?, ?, ?, ?, '["health","agent","index"]', 'offline', ?)`,
...
async function report(context: RequestContext): Promise<Response> {
  ...
  const status = ["completed", "failed"].includes(String(payload.status)) ? String(payload.status) : "failed";
  const now = epochSeconds();
  const job = await context.env.DB.prepare("SELECT user_id FROM hybrid_jobs WHERE id = ? AND device_id = ?")
    .bind(String(payload.job_id || ""), device.id).first<{ user_id: string }>();
  ...
  ).bind(status, JSON.stringify(payload.result ?? {}).slice(0, 100_000), now, now, String(payload.job_id), device.id).run();
  await notifyUser(context.env, job.user_id, {
    type: "bridge_job", title: status === "completed" ? "งานบนคอมเสร็จแล้ว" : "งานบนคอมไม่สำเร็จ",
