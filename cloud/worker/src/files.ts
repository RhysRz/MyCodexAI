...
async function createUpload(context: RequestContext): Promise<Response> {
  ...
  if (!Number.isFinite(size) || size < 1 || size > MAX_FILE_BYTES) {
    return errorJson("ไฟล์ต้องมีขนาดไม่เกิน 10 MB", 413);
  }
  ...
  if (Number(usage?.total || 0) + size > MAX_USER_BYTES) {
    return errorJson("พื้นที่ไฟล์ชั่วคราวของบัญชีครบ 50 MB แล้ว กรุณาลบไฟล์เก่า", 413);
  }
  ...
  await context.env.DB.prepare(
    `INSERT INTO cloud_files (id, user_id, name, media_type, size_bytes, chunk_count, status, created_at, expires_at)
     VALUES (?, ?, ?, ?, ?, ?, 'uploading', ?, ?)`,
  ).bind(row.id, row.user_id, row.name, row.media_type, row.size_bytes, row.chunk_count, row.created_at, row.expires_at).run();
  return json({ ...publicFile(row), chunk_bytes: CHUNK_BYTES }, 201);
}
...
async function listFiles(context: RequestContext): Promise<Response> {
  ...
  return json({ files: (rows.results || []).map(publicFile), chunk_bytes: CHUNK_BYTES, max_file_bytes: MAX_FILE_BYTES });
}
...
async function runnerDownload(context: RequestContext, id: string): Promise<Response> {
  ...
  ).bind(id).all<{ chunk_index: number; contents: ArrayBuffer }>();
